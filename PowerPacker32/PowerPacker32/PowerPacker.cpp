...
void CPowerPacker::UnPowerpack()
{
	...
	for(;;) 
	{
		...
		unsigned char *str = m_out_list.pos + ptr;
		while(len > 0) 
		{
...
void CPowerPacker::LoadBuffer(const unsigned char *pData, const ulong nSize)
{
	...
	m_in_list.ptr = (unsigned char*)malloc(m_in_list.size);
	...
	m_out_list.ptr = (unsigned char*)malloc(m_out_list.size);
	if (m_out_list.ptr == 0) 
	{
...
const  char* CPowerPacker::Save2()
{
	CAnsiFile OutputFile("dss", true);
	return OutputFile.writeBD(m_out_list.ptr, m_out_list.size);
}
